...
static void	echoing(t_cmd *cmd, t_shell *shell)
{
	...
	while (cmd->args[++i])
	{
		...
		else if ((quote_type == 0 || quote_type == 1) && strstr(cmd->args[i],
				"$") != NULL)
...
static void	do_echo(char *arg, int a, t_cmd *cmd)
{
	...
	if (a == 1) // Caso 1: * (todos ficheiros visíveis)
	{
		while (entry)
		{
			...
			if (entry->d_name[0] != '.')
			// Ignora os fichiros ocultos (os q começam com .)
			{
				if (first == false)
					ft_putchar_fd(' ', cmd->fd[1]);
	...
	{
		suffix = strndup(arg + 1, ft_strlen(arg) - 2);
		// Remove "*"" inicial e final
		suffix_len = ft_strlen(suffix);
		while (entry)
		{
			name_len = ft_strlen(entry->d_name);
			if (name_len > suffix_len && strncmp(entry->d_name + name_len
					- suffix_len, suffix, suffix_len) == 0)
			{
				if (!first)
					ft_putchar_fd(' ', cmd->fd[1]);
				ft_putstr_fd(entry->d_name, cmd->fd[1]);
	...
	// Caso 3: prefixo* (começa com padrão) // ta a dar erro
	{
	...
	else if (a == 4) // Caso 4: *padrão* (contém padrão no meio)
						// ta a dar erro
	{
