...
static void	expand_recursive(char **comps, int lvl, const char *base, t_collect *out);
...
void	ft_new_wildcard(t_cmd *cmd, t_shell *shell)
{
	...
	while (cmd->args[i])
	{
		...
		while (j < coll.len)
		{
			...
			if (j + 1 < coll.len) //add functions to ft_lib
				ft_strcat(out, " ");
			free(coll.arr[j]);
			j++;
		}
...
static bool match_pat(const char *name, const char *pat)
{
	...
	bool must_end   = pat[ft_strlen(pat)-1] != '*';
	...
	{
		char *p = ft_strstr(name + pos, chunks[idx]);
		if (!p)
		{
			free_strarr(chunks);
			return false;
	...
	if (must_end && nch > 0)
	{
		int len = ft_strlen(chunks[nch-1]);
		int nl  = ft_strlen(name);
...
static void collect_add(t_collect *c, char *s)
{
	...
	{
		c->cap *= 2;
		c->arr = realloc(c->arr, sizeof(char*) * c->cap); // tenho que arranjar outra solucao ao realloc
	}
	c->arr[c->len++] = s;
}
...
static void expand_recursive(char **comps, int lvl, const char *base, t_collect *out)
{
	...
	while ((e = readdir(d))) // posso usar readdir??
	{
		if (e->d_name[0] == '.')
			continue;
